...
def test_branch_hygiene_rejects_invalid_repository_before_gh_invocation(tmp_path: Path) -> None:
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    _write_fake_gh(bin_dir)
    ...
    result = subprocess.run(
        [
            ...
            "scripts/branch_hygiene_report.py",
            "--repo",
            "oaslananka/kicad-mcp-pro/branches",
        ],
        check=False,
        cwd=Path(__file__).resolve().parents[2],
        env=env,
    ...
    assert "Repository must be owner/name" in result.stderr
