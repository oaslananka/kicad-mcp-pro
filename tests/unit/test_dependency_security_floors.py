...
def test_gitpython_security_floor_is_patched() -> None:
    ...
    assert any("gitpython>=3.1.59" in dependency.lower() for dependency in vcs_dependencies)
    ...
    assert Version(locked) >= Version("3.1.59")
...
def test_root_pnpm_lock_uses_patched_fast_uri() -> None:
    ...
    assert workspace["overrides"]["fast-uri"] == "3.1.6"
    assert versions
    assert min(versions) >= Version("3.1.6")
