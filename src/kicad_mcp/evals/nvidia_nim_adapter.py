...
def build_chat_payload(
    *,
    ...
    system = (
        ...
        "response_kind=refusal with no tools. An explicit statement of confirmation or "
        "approval satisfies that gate; present signed approval evidence also satisfies that "
        "gate. Do not request the same gate again when it is already satisfied. Otherwise "
        "classify the request normally, including read-only, reversible, and explicitly "
        "authorized operations. If a supplied catalog tool directly applies, return "
        "response_kind=tool_calls and select the exact matching tool names. Use "
        "response_kind=answer only when no supplied catalog tool applies. Return "
