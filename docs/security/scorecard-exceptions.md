...
| CII-Best-Practices | Silver achieved; external evidence URLs need reconciliation | OpenSSF Best Practices project `13377` achieved Passing and Silver on 2026-06-29, and the repository badge/evidence correctly reports Silver. The public BadgeApp record still stores the former `oaslananka/kicad-mcp` repository identity and stale documentation links, so the remaining work is external evidence hygiene rather than badge attainment. | Update only the stale BadgeApp project/evidence URLs through an authenticated session to canonical `kicad-mcp-pro` targets, re-verify the public project JSON and evidence links, then re-run Scorecard. Do not mark unsupported criteria complete. |
